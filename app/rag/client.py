...
def get_qdrant_client(settings: Settings | None = None) -> QdrantClient:
    ...
    When EMBEDDING_PROVIDER=qdrant (default), cloud_inference=True so query
    ``Document`` objects are embedded server-side — no local model weights.
    """
    global _client
    settings = require_qdrant_settings(settings)
    if _client is None:
        use_cloud_inference = settings.embedding_provider.strip().lower() == "qdrant"
